...
void player_wait_for_frame(Player *player, int64_t stream_time, int stream_no) {
	...
	for (;;) {
		...
		LOGI("player_wait_for_frame[%d], current time: [%llu] , stream time : [%llu] ,  sleep time [%llu]",
				stream_no, current_video_time, stream_time, sleep_time);
...
void decode_video(Player *player, AVPacket *packet) {
	...
	if (got_frame) {
		//lock
		//LOGI("height: %d , width %d ", codec_ctx->height, codec_ctx->width);
		//设置缓冲区的属性（宽、高、像素格式）
...
JNIEXPORT void JNICALL Java_com_dd_ffmpeg_DDPlayer_play(JNIEnv *env,
